...
class ViewHostPassenger extends StatefulWidget {

  const ViewHostPassenger({ Key? key,required this.request }) : super(key: key);
  final CarpoolObject request;
  @override
  _ViewHostPassengerState createState() => _ViewHostPassengerState();
}

class _ViewHostPassengerState extends State<ViewHostPassenger> {
List <UserData> passengerlist=<UserData>[];
  ...
  Widget build(BuildContext context) {
   ...
   return CircularProgressIndicator();
          ...
          for(int h=0;h<docnum;h++)
          {
            if(snapshot.data!.data()!["Host ID"]==QS.docs.elementAt(h).id)
            {
              host=new  UserData.set(
                        uid: temp.elementAt(h).id,
                        name: temp.elementAt(h).data()["name"],
                        icNo: temp.elementAt(h).data()["icNo"],
                        gender: temp.elementAt(h).data()["gender"],
                        phone: temp.elementAt(h).data()["phone"],
                        occupation: temp.elementAt(h).data()["occupation"],
                         license: temp.elementAt(h).data()["havelicense"],
                         licenseType: temp.elementAt(h).data()["licenseType"],
                         address: temp.elementAt(h).data()["address"],
                         imageurl: temp.elementAt(h).data()["url"],
                          haveCar: temp.elementAt(h).data()["haveCar"],
                          age:temp.elementAt(h).data()["age"]);
